...
#     start_time = time.time()
#     new_size = (imgs[1].shape[0]//2, imgs[1].shape[1]//2)S
#     res_im = []
#     for im in imgs:
...
def res_skimage(imgs):
    start_time = time.time()
    new_size = [imgs[1].shape[0]//2, imgs[1].shape[1]//2,len(imgs)]
    new_size = tuple(new_size)
    res_im = []
    image_resized = resize(imgs,(12,12,200), anti_aliasing=True)
    print("The program took: "+str(time.time()-start_time)+ " secs")
...
pi_list = []
for num in nums:
    pi_list.append(approximate_pi_numba(num))
print(f"The program took {time.time()-start_time} secs")
#####################Task 4################################
num_x = np.log(nums)
plt.scatter(nums,pi_list,label="approximation")
plt.axhline(y=math.pi,color='r',label="Value of pi from math.pi")
